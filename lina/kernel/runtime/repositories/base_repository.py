...
TRecord = TypeVar("TRecord", bound=Base)


class SQLAlchemyRepository(EngagementScopedRepository[TEntity], Generic[TEntity, TRecord]):
    ...
    async def get(self, entity_id: str) -> TEntity | None:
        record = await self._session.get(self._record_cls, entity_id)
        if record is None:
            return None
        return record_to_entity(record, self._entity_cls)

    async def add(self, entity: TEntity) -> TEntity:
        existing = await self._session.get(self._record_cls, entity.id)
        if existing is not None:
            raise ConflictError(f"{self._entity_name} with id={entity.id} already exists.")
        kwargs = entity_to_record_kwargs(entity, extra_columns=self._column_extractor(entity))
    ...
    async def update(self, entity: TEntity) -> TEntity:
        record = await self._session.get(self._record_cls, entity.id)
        if record is None:
            raise NotFoundError(self._entity_name, entity.id)
        kwargs = entity_to_record_kwargs(entity, extra_columns=self._column_extractor(entity))
    ...
    async def list(
        ...
        *,
        ...
        entities = [record_to_entity(r, self._entity_cls) for r in records]
    ...
    def _apply_python_filters(
        self, entities: list[TEntity], filters: dict[str, object]
    ) -> list[TEntity]:
        """Filters that don't map to a denormalized SQL column are applied
        in-memory against the deserialized entity (acceptable for a local,
        single-operator SQLite deployment at LINA's target scale).
        """
