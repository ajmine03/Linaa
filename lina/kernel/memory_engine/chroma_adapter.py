...
class ChromaMemoryEngine(MemoryEnginePort):
    ...
    async def upsert(self, collection: str, records: list[MemoryRecord]) -> None:
        ...
        if texts_needing_embedding:
            try:
                ...
                raise VectorStoreError(f"Failed to embed records for upsert: {exc}") from exc
            for text, vector in zip(texts_needing_embedding, result.vectors, strict=True):
        ...
        embeddings = [
            r.embedding if r.embedding is not None else embeddings_by_text[r.text] for r in records
        ]
        ...
        try:
            ...
            raise VectorStoreError(f"ChromaDB upsert failed for collection '{collection}': {exc}") from exc

        logger.debug("memory_engine.upserted", collection=collection, count=len(records))
    ...
    async def query(
        ...
        *,
        ...
        try:
            ...
            raise VectorStoreError(f"Failed to embed query text: {exc}") from exc
        ...
        try:
            ...
            raise VectorStoreError(f"ChromaDB query failed for collection '{collection}': {exc}") from exc
        ...
        for rid, doc, meta, dist in zip(ids, documents, metadatas, distances, strict=True):
            score = 1.0 - dist if self._config.distance_metric == "cosine" else -dist
            record = MemoryRecord(id=rid, text=doc or "", metadata=dict(meta or {}))
    ...
    async def delete_collection(self, collection: str) -> None:
        ...
        try:
            ...
            raise VectorStoreError(f"ChromaDB collection delete failed: {exc}") from exc

    async def get_by_id(self, collection: str, record_id: str) -> MemoryRecord | None:
        chroma_collection = self._get_collection(collection)
        try:
            raw = chroma_collection.get(ids=[record_id], include=["documents", "metadatas"])
        except Exception as exc:  # noqa: BLE001
